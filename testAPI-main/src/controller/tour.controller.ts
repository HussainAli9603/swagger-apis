...
export class TourController {
    ...
    async saveTour(@Body() tours: ITourRequest): Promise<any> {
        ...
        if (date.toString() != 'Invalid Date') {
            ...
            console.log("Alloted bus" + alloted_busses);

            const registeredTour: any = await new MainTour().saveTour(tours, alloted_busses);
            if (!registeredTour)
    ...
    private AllotBusses(available_busses: Array<any>, tour_participants: number) {
        ...
        console.log("buss_subsit" + buss_subset);
        ...
        console.log("mapped_subsets" + mapped_subsets);
